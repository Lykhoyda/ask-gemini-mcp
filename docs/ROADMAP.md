...
- [x] **npm 9 EUNSUPPORTEDPROTOCOL workspace:*** — `npx -y ask-llm-mcp` failed under Claude Desktop's Node 18 / npm 9.7.1 because the published package.json contained raw `workspace:*` strings that npm 9's arborist rejects during manifest parsing (before `bundledDependencies` can help). Added `scripts/prepack-bundle.mjs` + `scripts/postpack-restore.mjs` that rewrite `workspace:*` → `*` at pack time and restore the workspace protocol in the dev working tree after pack. Published `ask-llm-mcp@0.2.6` / `ask-codex-mcp@0.2.6` / `ask-ollama-mcp@0.2.6` / `ask-gemini-mcp@1.5.6` with the fix (ADR-052)
